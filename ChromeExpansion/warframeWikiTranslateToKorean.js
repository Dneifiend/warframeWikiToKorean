...
`;
      ...
      if (input.value.length > 1) {
        ...
        var res = searchResult.items
          ...
          .map((result, resultIdx) => {
            ...
            li.dataset.en = Object.keys(result)[0].replace(
              /(<b>)|(<\/b>)/g,
              ""
            );
            li.tabIndex = 0;
    ...
    document
      ...
      .forEach((ele, idx) => {
        if (ele.childElementCount === 0) {
          var kr =
            data[ele.textContent.trim().replace(/\s/g, " ").toUpperCase()];
          if (kr) {
            ele.textContent = kr;
          }
        }
  ...
  transOb.observe(document.querySelector("body"), {
    subtree: true,
    childList: true,
  });
})();
